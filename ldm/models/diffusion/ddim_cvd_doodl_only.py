...
class DDIMCVDDOODLSampler(object):
    ...
    def ddim_sampling(self, cond, shape,
        ...
        if x_T is None:
            if self.rnd is not None:
                img = self.rnd.randn(shape, device=device)
            else:
        ...
        img = self.doodl(img)
        self.save_tensor(img, "./doodl_inits")
    ...
    def loss_fn(self, x_cvd_d):
        x_cvd_d_sim = run_sim(x_cvd_d, 1.0, x_cvd_d.device,
                              cvd_type=self.cvdkwargs['cvd_type'])

        alpha = self.cvdkwargs['cvd_alpha']
        scale = self.cvdkwargs['gs']
        return scale * (alpha * color_info_loss(x_cvd_d, x_cvd_d_sim) + (1 - alpha) * MS_SSIM_loss(x_cvd_d, x_cvd_d_sim).sum())
    ...
    def save_tensor(self, tensor, directory):
        # Ensure the directory exists
        os.makedirs(directory, exist_ok=True)

        # Regular expression to match the filenames
        pattern = re.compile(r"doodl_inits_(\d+)\.pt")

        # List all files in the directory
        files = os.listdir(directory)

        # Filter files based on the pattern and find the highest number
        ...
        filename = f"doodl_inits_{next_number}.pt"
    ...
    def doodl(self,
        ...
        """DOODL: Direct Optimization of Diffusion Latents from https://arxiv.org/abs/2303.13703"""
        ...
        with torch.enable_grad(): # important b/c don't have on by default in module
            for m in range(num_traversal_steps): # This is # of optimization steps
                ...
                with autocast(device):
                    ...
                    for i, step in enumerate(iterator):
                        ts = torch.full((latent.shape[0],), step, device=device, dtype=torch.float32)
                ...
                for im_i, im in enumerate(ims):
                    ...
                    # if im_i == 0:
                    #     PIL.Image.fromarray(convert(im)[0].numpy(), 'RGB').save(f'/root/doodlimgs/i_{m}.png')
