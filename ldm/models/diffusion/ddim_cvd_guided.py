...
class DDIMCVDSampler(object):
    ...
    def p_sample_ddim(self, x, x_pivot, c, t, index, repeat_noise=False, use_original_steps=False,
        ...
        if self.cvdkwargs['backward_iters'] > 0:
            ...
            for i in range(self.cvdkwargs['backward_iters']):
                ...
                cvd_loss = self.loss_fn(x0_full, x0_full_pivot, sqrt_one_minus_at)
                cvd_loss.backward()
    ...
    def loss_fn(self, x_cvd_d, x_pivot_d, nl):
        ...
        cvd_loss = alpha * color_info_loss(x_cvd_d, x_cvd_d_sim,  nl.squeeze()) + (1 - alpha) * MS_SSIM_loss(x_cvd_d, x_cvd_d_sim).sum()
