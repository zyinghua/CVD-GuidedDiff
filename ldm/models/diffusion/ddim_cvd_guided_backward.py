...
class DDIMCVDSampler(object):
    ...
    def p_sample_ddim(self, x, x_pivot, c, t, index, repeat_noise=False, use_original_steps=False,
        ...
        backward_g_mask = torch.zeros_like(e_t_cvd)
        backward_g_mask = backward_g_mask.detach().requires_grad_(True)
        optimizer_backward = Adam([backward_g_mask], lr=1e-3)

        for i in range(10):
            optimizer_backward.zero_grad()
            pred_x0_cvd = (x_cvd.detach() - sqrt_one_minus_at * e_t_cvd.detach()) / a_t.sqrt()
            pred_x0_cvd = pred_x0_cvd + backward_g_mask

            x0_full = self.decode_x0(pred_x0_cvd)
            cvd_loss = self.loss_fn(x0_full)
            cvd_loss.backward()
            optimizer_backward.step()

        pred_x0_cvd = (x_cvd - sqrt_one_minus_at * e_t_cvd) / a_t.sqrt()
        x_cvd = a_t.sqrt() * (pred_x0_cvd + backward_g_mask) + sqrt_one_minus_at * e_t_cvd
        e_t_cvd, attn_maps_cvd = self.model.apply_model(x_cvd, t, c)
