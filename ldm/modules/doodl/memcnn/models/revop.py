...
from ldm.modules.doodl.memcnn.models.affine import AffineCoupling
...
print("Using torch float32, if need more precision need to change custom_fwd")
...
class ReversibleBlock(InvertibleModuleWrapper):
    def __init__(self, Fm, Gm=None, coupling='additive', keep_input=False, keep_input_inverse=False,
        ...
        * 1  Memory efficient implementation, similar to approach in Gomez et al. 2017.
        ...
        """
        warnings.warn("This class has been deprecated. Use the more flexible InvertibleModuleWrapper class",
                      DeprecationWarning)
...
def is_invertible_module(module_in, test_input_shape, test_input_dtype=torch.float32, atol=1e-6, random_seed=42):
    ...
    with torch.no_grad():
        ...
        if not _check_inputs_allclose(_pack_if_no_tuple(module_in.inverse(*_pack_if_no_tuple(module_in(*test_inputs)))),
                                      test_inputs, atol=atol):
            return False
        ...
        if not _check_inputs_allclose(
            _pack_if_no_tuple(module_in(*_pack_if_no_tuple(module_in.inverse(*test_outputs)))), test_outputs,
            atol=atol):  # pragma: no cover
            return False
    ...
    def _test_shared(inputs, outputs, msg):
        ...
        if len(inputs) != len(shared):  # pragma: no cover
            warnings.warn(
                "Some inputs (*x) share the same tensor, are you sure this is what you want? ({})".format(msg))
        if len(outputs) != len(shared_outputs):
            warnings.warn(
                "Some outputs (*y) share the same tensor, are you sure this is what you want? ({})".format(msg))
