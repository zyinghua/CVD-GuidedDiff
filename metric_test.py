...
def all_tensor(imglist):
    ...
    for idx, imgpath in enumerate(imglist):
        # print('image:', imgpath)
        im = np.asarray(PIL.Image.open(imgpath).convert('RGB')).copy()
...
def run_res(imgList):
    for img in imgList:
        im = np.asarray(
            PIL.Image.open(f'/Users/jiangshuyi/USYD/Research/hfai/testdata/{img}.png').convert('RGB')).copy()
        res = get_tensor(im)
...
def color_info_loss(im1, im2):
    ...
    im1_a, im1_b = rgb2ab(im1)
    im2_a, im2_b = rgb2ab(im2)

    return torch.mean(torch.sqrt(torch.pow(im1_a - im2_a, 2) + torch.pow(im1_b - im2_b, 2)))

def run_single(img, degree):
...
def run_cvd_metrics(img, degree, type='DEUTAN'):
    ...
    print(f'color loss of {img}.png', colorl)
    VGG_loss = VGGPerceptualLoss()
    Loss = VGG_loss((res + 1) / 2, (res_sim + 1) / 2)
    print(f'VGG of {img}.png', Loss)
    print('*' * 50)
